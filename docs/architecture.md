...
- Force RuneLite profile settings on every profile, from the **Profile** tab: remove keys by prefix and write key=value pairs before each launch. A GPU block is included as an example of the form; nothing applies unless turned on.
